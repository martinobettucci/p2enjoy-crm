...
// @verifies CRM-022 (docs/BACKLOG.md) — auteurs nommés et avatars embarqués
...
test.describe('fil chargé (réponse réseau substituée, docs/DESIGN_SYSTEM.md §12.5)', () => {
	...
	test('les auteurs et leurs avatars sont affichés, jamais leurs identifiants', async ({ page }) => {
		await servirEcran(page)
		await page.goto(ADRESSE)
		await expect(fil(page)).toContainText('Camille Aubert')
		await expect(fil(page)).toContainText('Driss Lemoine')
		await expect(fil(page).getByTestId('avatar')).toHaveCount(3)
		await expect(fil(page)).not.toContainText('5eed0000-0000-4000-8000-000000000011')
		await expect(fil(page)).not.toContainText('5eed0000-0000-4000-8000-000000000012')
	})
