...
// @verifies CRM-022 (docs/BACKLOG.md) — auteurs et acteurs nommés par relations embarquées
...
/**
 * Un commentaire et cinq événements, un par famille et un type par forme de `payload`.
 *
...
const EVENEMENTS_SERVIS = [
	...
	},
	{
		...
		actor_id: '5eed0000-0000-4000-8000-000000000011',
		acteur: CAMILLE,
		payload: { from_channel_id: 'channel-1', to_channel_id: 'channel-2' },
...
test.describe('le fil unifié (réponses substituées, docs/DESIGN_SYSTEM.md §12.5)', () => {
	test('range les faits et la parole dans un seul fil, du plus ancien au plus récent', async ({
		...
		await expect(lignes.nth(5)).toContainText('Dossier changé')
	...
	test('nomme l’auteur et les acteurs consentis, jamais leurs identifiants', async ({ page }) => {
		await servirEcran(page)
		await page.goto(ADRESSE)

		await expect(fil(page).getByText('Camille Aubert', { exact: true })).toBeVisible()
		await expect(fil(page).getByText('par Camille Aubert', { exact: true })).toHaveCount(4)
...
test.describe('les filtres (docs/DESIGN_SYSTEM.md §5.11)', () => {
	test('cinq bascules, toutes actives, dont le compte suit la SOURCE', async ({ page }) => {
		await page.setViewportSize({ width: 1440, height: 900 })
		await servirEcran(page)
		await page.goto(ADRESSE)

		const bascules = filtres(page).getByRole('button')
		await expect(bascules).toHaveCount(5)
		...
		await expect(filtres(page).getByRole('button', { name: /Organisation/ })).toContainText('1')
	...
	test('éteindre une famille la masque, et le compte ne bouge pas', async ({ page }) => {
		...
		await servirEcran(page)
		await page.goto(ADRESSE)
		await expect(fil(page).getByRole('listitem')).toHaveCount(6)
