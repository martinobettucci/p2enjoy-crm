...
// @verifies docs/INCONSISTENCY_REPORT.md INC-037 (`require_fields` vide), INC-021 (aucun écran)
...
test.describe('C1 — la copie, et ce qu’elle contient (§4.9, lignes a, b, l)', () => {
	test('ligne a — un administrateur copie le workflow, avec ses étapes et ses arêtes', async ({
		...
		try {
			...
			// Le workflow du seed porte sept étapes et dix transitions (§3.9). La copie doit en
			...
			const aretes = await request.get(
				`${TRANSITIONS}?workflow_id=eq.${copieId}&select=from_step_id,to_step_id,label,require_comment,require_fields`,
				{ headers: enTetesAuthentifies(jeton) },
			)
			...
			expect(listeAretes, 'dix arêtes copiées').toHaveLength(10)
			expect(
				listeAretes.filter((t) => t.require_comment),
				'dont les quatre qui exigent un commentaire',
			).toHaveLength(4)
...
test.describe('C3 — ce qui ne se copie pas (§4.9, lignes f, g, h, i, j, k)', () => {
	...
	test('ligne i — une copie ne se copie pas', async ({ request }) => {
		...
		const derivee = await request.get(
			`${WF}?derived_from_workflow_id=eq.${WORKFLOW_SEED}&track_id=eq.${TRACK_CONSEIL}&select=id,scope`,
			{ headers: enTetesAuthentifies(jeton) },
		)
