...
// @verifies docs/INCONSISTENCY_REPORT.md INC-037 (la copie n'emporte pas les champs)
...
test.describe('F0 — le seed est dans l’état que le §2.10 déclare', () => {
	...
	// INC-037, constatée hors interface comme elle l'est en pgTAP. La copie du seed n'a aucun champ.
	test('INC-037 — la copie de portée `track` ne porte aucun champ', async ({ request }) => {
		const copie = await request.get(
			`/rest/v1/workflows?select=id&derived_from_workflow_id=eq.${WORKFLOW_GLOBAL}`,
			{ headers: enTetesService() },
		)
		...
		expect(champs.status()).toBe(200)
		expect(await champs.json()).toEqual([])
	})
})
