...
//     La table est peuplée par le seed — quatorze valeurs —, et l'état est d'abord constaté avec la
...
test.describe('V3 — validation par type', () => {
	...
	test('o) une valeur croisant card et champ de deux workflows → 400, 23503', async ({
		...
		// Le `workflow_id` de la copie de portée track est tiré au hasard : il est lu, non deviné.
		const copies = await request.get('/rest/v1/workflows?scope=eq.track&select=id', {
			headers: enTetesService(),
		})
...
test.describe('V4 — la sixième vérification', () => {
	...
	test('q ter) l’UNION : `require_fields` de la transition exige, l’étape cible n’exigeant rien', async ({
		request,
	}) => {
