...
// @verifies docs/SPEC-cards.md §14.4 (les huit types), §14.5 (triggers `SECURITY DEFINER`),
...
test.describe('CRM-044 — la mémoire d’une affaire, hors interface', () => {
	...
	test('m — le seed a fait naître neuf cards, et son histoire ne cesse plus de croître', async ({
		request,
	}) => {
		// Les neuf cards du seed sont ÉNUMÉRÉES, jamais filtrées par motif : `?card_id=like.5eed*`
		...
		// pile.
		const CARDS_DU_SEED = Array.from(
			{ length: 9 },
		...
		// pose 27 événements sur une base fraîchement seedée — `scripts/verify-timeline.sh` le
		...
		expect(evenements.filter((e) => e.type === 'created')).toHaveLength(9)
		expect(evenements.filter((e) => e.type === 'created' && e.actor_id !== null)).toHaveLength(0)
		expect(evenements.filter((e) => e.type === 'field_changed').length).toBeGreaterThanOrEqual(14)
