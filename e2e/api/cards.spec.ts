...
/**
 * Les neuf cards du seed (`docs/SPEC-cards.md` §9).
 *
...
test.describe('C0 — le seed est dans l’état que le §9 déclare', () => {
	test('neuf cards, dont une archivée et une en corbeille, toutes avec une adresse conforme', async ({
		request,
	}) => {
		...
		expect(new Set(cards.map((c) => c.email_local_part)).size).toBe(9)
	})

	test('INC-046 — aucune card seedée dans « prospection », et le motif est mesuré', async ({
		request,
	}) => {
		// `prospection` est le seul channel que le seed repointe : la section 4 le ramène au
		// workflow global, la section 7 le rattache à la copie de portée track. La clé composite
		// refuse le premier geste dès qu'une card y vit — mesuré, le seed échoue alors en
		// section 4 (docs/SPEC-cards.md §9.1).
		const reponse = await request.get(
			`${CARDS}?channel_id=eq.${CH_PROSPECTION}&${FILTRE_SEED}&select=id`,
			{ headers: enTetesService() },
		)
		expect(reponse.status()).toBe(200)
...
test.describe('C5 — lecture : lignes o à q du §8.1', () => {
	...
	test('p — l’admin voit tout : un administrateur n’est jamais restreint', async ({ request }) => {
		...
		expect(vu.status()).toBe(200)
		expect(await vu.json()).toHaveLength(9)
	})
