...
test.describe('les deux lectures de la vue liste (§12.3)', () => {
	test('la page rapporte les colonnes du produit, et la pile les rend toutes', async ({ request }) => {
		...
		for (const ligne of lignes) {
			// Chacune des sept colonnes demandées est présente, y compris celles qui valent `null` :
			// une colonne absente de la réponse et une colonne nulle ne se distinguent pas à l'œil.
			for (const colonne of COLONNES_CARD_LISTE.split(',').map((nom) => nom.trim())) {
	...
	// Le nom du responsable n'est lisible par personne (INC-014) : la colonne n'est donc pas
	// demandée du tout, et ce contrôle constate que l'omission n'est pas un oubli.
	test('la lecture ne demande ni `owner_id`, ni `position`, ni `description`', () => {
		for (const absente of ['owner_id', 'position', 'description', 'health_score', 'search_tsv']) {
...
test.describe('la pagination et son `416` (§12.6)', () => {
	...
	// MESURÉ : `count=planned` rend 1 là où la table en porte 3. Une pagination bâtie dessus
