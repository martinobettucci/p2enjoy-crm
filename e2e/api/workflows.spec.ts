...
// @verifies docs/INCONSISTENCY_REPORT.md INC-029 (channels rattachés), INC-033 (liaison CRM-018)
...
test.describe('W0 — le seed a réellement posé le workflow par défaut', () => {
	...
	test('un workflow global et par défaut, sept étapes, onze transitions', async ({ request }) => {
		...
		expect(reponseEtapes.status()).toBe(200)
		...
		const reponseTransitions = await request.get(
			`${TRANSITIONS}?select=id,require_comment&workflow_id=eq.${WORKFLOW_SEED}`,
			{ headers: enTetesService() },
		)
		expect(reponseTransitions.status()).toBe(200)
		const transitions = (await reponseTransitions.json()) as {
			id: string
			require_comment: boolean
		}[]
		expect(transitions).toHaveLength(11)
		expect(transitions.filter((t) => t.require_comment)).toHaveLength(5)
		...
		// deux motifs ont disparu : la sixième vérification de `move_card` lit désormais la liaison,
		// et le seed en porte exactement UNE (docs/SPEC-seed.md §2.13). Elle est ciblée par ses deux
		// identifiants, de sorte qu'une sonde concurrente ne rende pas ce scénario instable.
		const liens = await request.get(
			`${REQUIRED_FIELDS}?transition_id=eq.${TRANSITION_REALISATION}`
			+ `&field_id=eq.${CHAMP_LIEN_PROPOSITION}&select=transition_id,field_id`,
			{ headers: enTetesService() },
		)
		expect(liens.status()).toBe(200)
		expect(
			(await liens.json()) as unknown[],
			'CRM-018 : la liaison exacte du seed existe, indépendamment des sondes concurrentes',
		).toHaveLength(1)
	})
