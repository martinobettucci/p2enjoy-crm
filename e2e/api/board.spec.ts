...
test.describe('B2 — la lecture des transitions rend les gestes atteignables (§7.2, lecture n° 2)', () => {
	test('les dix transitions du workflow standard', async ({ request }) => {
		const reponse = await request.get(
	...
	test('quatre transitions exigent un motif, et le seed les porte en permanence', async ({ request }) => {
		const reponse = await request.get(
			`${TRANSITIONS}?select=${encodeURIComponent(COLONNES_TRANSITION)}&workflow_id=eq.${WORKFLOW_GLOBAL}`,
			{ headers: enTetesAuthentifies(jetonAdmin) },
		)
		const transitions = (await reponse.json()) as { require_comment: boolean }[]
		expect(transitions.filter((transition) => transition.require_comment)).toHaveLength(4)
	})
...
test.describe('B5 — ce que le board ne peut pas montrer, et c’est mesuré (§7.4)', () => {
	// La raison pour laquelle aucune carte n'affiche d'avatar : `profiles` reste en refus par
	// défaut, INC-014. Mesuré avec le jeton de l'ADMINISTRATRICE, pas d'un profil restreint —
	// c'est ce qui distingue un refus de politique d'un manque de droit.
	test('le nom du responsable n’est lisible par personne, pas même par l’administratrice', async ({
		request,
	}) => {
		const reponse = await request.get(`${PROFILS}?select=id,full_name`, {
			headers: enTetesAuthentifies(jetonAdmin),
		})
		expect(reponse.status()).toBe(200)
		expect((await reponse.json()) as unknown[]).toEqual([])

		// Et pourtant `owner_id` est lisible : la carte a l'identifiant et pas le nom. Afficher
		// l'identifiant serait une valeur par défaut trompeuse (CLAUDE.md §18).
		const cards = await request.get(
			`${CARDS}?select=owner_id&channel_id=eq.${CHANNEL_GRANDS_COMPTES}&owner_id=not.is.null`,
			{ headers: enTetesAuthentifies(jetonAdmin) },
		)
		expect(((await cards.json()) as unknown[]).length).toBeGreaterThan(0)
	})
