...
// @spec docs/SPEC-cards.md §14.4 (les huit types), §14.6 (payloads, et les libellés absents),
 ...
 * `actor_id` l'est **et pourtant n'est jamais affichée** — INC-014 rend `profiles` illisible, et le
 * §5.11 du design system ne rend aucun nom d'acteur. Elle est lue parce que le jour où les profils
 * seront lisibles, c'est le seul champ qui manquera.
 */
export const COLONNES_EVENEMENT = 'id, card_id, type, actor_id, payload, created_at'
...
/** Les huit types que la base accepte (docs/SPEC-cards.md §14.4). */
...
/**
 * Les quatre familles de filtres du §5.11 — et pas huit.
 *
 ...
 */
export const FAMILLES = ['discussion', 'etapes', 'champs', 'cycle'] as const
...
export function compterParFamille(lignes: readonly LigneFil[]): Readonly<Record<Famille, number>> {
	const comptes: Record<Famille, number> = { discussion: 0, etapes: 0, champs: 0, cycle: 0 }
