...
| `scripts/verify-recherche.sh` | Rejoue les preuves de la **recherche globale en base** (`CRM-065` tranche 1) : traçabilité, le vocabulaire `app.francais_sans_accent` **et sa contre-épreuve** — `french` seule ne trouve pas « créance » sur « creance » —, les cinq index GIN et le fait qu'**aucune colonne** n'a été ajoutée aux tables cherchées, la forme de `public.recherche_globale` mesurée dans le catalogue (`security invoker`, `stable`, `anon` révoqué **nommément**), ce que la tranche **n'a pas touché** — les cinq politiques de lecture, le compte de politiques, `default_text_search_config` —, le comportement sous les trois profils du seed avec leurs refus **et leur contre-épreuve**, **sept dégradations réelles** dont une qui rend la fonction `SECURITY DEFINER` en laissant tout le reste debout — la recherche répond toujours, mais pour `postgres`, qui traverse toute la RLS —, et restauration de la base **constatée** plutôt que supposée | **disponible** |
| `scripts/verify-recherche.sh --rapide` | Omet le contrat d'API Playwright, et l'annonce dans sa sortie ; les mesures et les sept dégradations de la base restent jouées | **disponible** |
...
production : son secret, `OIDC_CLIENT_SECRET`, est tiré au hasard par `./runDev.sh` et substitué
...
│   ├── spark/amorcer-espace.sh Premier espace de production et l'attente de son administrateur
