...
--       128 (identifiant mort ignoré), 129 (un champ archivé n'exige rien),
...
--       occurrence), INC-033 (aucune intégrité sur un `uuid[]`), INC-047 (**close par cette
--       migration**), INC-053 (`user` et `contact` non résolus)
--
...
begin
	...
	--   * un champ ARCHIVÉ, quelle que soit sa règle et quel que soit `require_fields`
	...
	--   * un identifiant de `require_fields` que la jointure NE RÉSOUT PAS (décision 128). La
	--     jointure interne ci-dessous le fait tomber naturellement. Dénoncer bloquerait
	--     définitivement une transition pour une erreur de saisie d'administrateur, qu'aucun
	--     utilisateur ne peut corriger depuis le produit ; ignorer ne fait que ne pas exiger ce que
	--     personne ne peut renseigner — les clés composites de la section 3 l'interdisent.
	--
	...
	select array_agg(f.key order by f.position, f.key)
	 ...
	 where f.workflow_id = v_card.workflow_id
	   ...
	   and (
	       ...
	       or f.id = any (coalesce(v_transition.require_fields, '{}'::uuid[]))
	   )
	   and not exists (
	       select 1
