...
--     rend `NULL`. Aucune table n'est créée par anticipation ; `require_fields` est copié tel quel,
--     ce qui reste correct après `CRM-035`, les identifiants qu'il porte désignant des champs du
--     **workspace** que la copie ne change pas. INC-037 ;
--
...
begin
	...
	--
	-- `require_fields` est copié tel quel. Il est vide partout aujourd'hui, `form_fields` n'existant
	-- pas (INC-037) ; il restera correct ensuite, ses identifiants désignant des champs du
	-- **workspace**, que la copie ne change pas.
	insert into public.workflow_transitions (
		workflow_id, workspace_id, from_step_id, to_step_id,
		label, require_comment, require_fields
	)
	select v_copie_id, t.workspace_id, copie_depuis.id, copie_vers.id,
	       t.label, t.require_comment, t.require_fields
