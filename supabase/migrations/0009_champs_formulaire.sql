...
--       INC-033 (`require_fields` sans intégrité référentielle), INC-037 (la copie n'emporte pas
--       les champs), INC-043 (`CRM-034` sans cible)
--
...
--
--   * **la copie des champs vers un track**. `copy_workflow_to_track` (`CRM-032`) n'en copie
--     aucun : elle a été écrite quand `form_fields` n'existait pas. Le comportement reste
--     **inchangé** — INC-037, arbitrage attendu du responsable, décision 93. L'écart est figé par
--     des assertions dans les suites `0007`, `0008` et `0010`, pas par ce commentaire ;
--
--   * **toute intégrité référentielle sur `require_fields`**, qui pourrait désormais désigner des
--     champs réels. MESURÉ à `CRM-031`, et propriété du type et non différé : PostgreSQL refuse une
--     clé étrangère depuis une colonne `uuid[]`. INC-033.
--
-- ---------------------------------------------------------------------------------------------
