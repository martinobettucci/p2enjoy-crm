...
--           §5 (« active »), §14.4 (neuf types), §14.6 (payload), §14.8 (immuabilité)
...
-- =============================================================================================
-- 2. Le vocabulaire passe à NEUF valeurs — docs/SPEC-cards.md §14.4
-- =============================================================================================
...
select is(
	...
	'Le `CHECK` compte NEUF valeurs : `channel_changed` s''ajoute aux huit de `CRM-044`. AUCUN '
	'GARDE-FOU N''A ÉTÉ DÉCLENCHÉ PAR CET AJOUT, ET C''EST UNE VÉRIFICATION QUI L''A ÉTABLI : '
	'`0018_timeline.test.sql` éprouve ses huit types EN ÉCRIVANT, un à un, et n''a jamais compté '
	'l''énumération. Le mécanisme de la décision 51 ne pouvait donc pas jouer ici — cette assertion '
	'est le premier recensement du vocabulaire, et elle échouera à la dixième valeur');

-- LE MÉCANISME DE `CRM-044` EST RECONDUIT, ET IL DOIT LE RESTER. `mail_received` et `mail_sent`
-- demeurent refusés : le jour où `CRM-054` les écrira, la base le lui rappellera par un `23514`.
select throws_ok(
	$$insert into public.card_events (card_id, workspace_id, type)
	  values ('5eed0000-0000-4000-8000-0000000000c5',
	          '5eed0000-0000-4000-8000-000000000001', 'mail_received')$$,
	'23514',
	null,
	'`mail_received` reste REFUSÉ par le `CHECK` : `CRM-054` devra étendre l''énumération dans la '
	'même migration que son trigger, exactement comme cette unité vient de le faire');
...
-- =============================================================================================
-- 9. Ce que cette unité ne livre pas, figé pour que son unité le retourne
-- =============================================================================================
...
select hasnt_function('public', 'change_channel_workflow', array['uuid', 'uuid'],
	'AUCUNE RPC ne change le workflow d''un channel peuplé : c''est l''option 2 de l''arbitrage '
	'd''INC-046, et elle reste une unité de backlog qui n''existe pas (§6.11)');
