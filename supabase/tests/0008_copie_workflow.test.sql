...
--           copié), §4.6 (vue de divergence), §4.7 (privilèges), §4.8 (champs non copiés)
...
-- @verifies docs/INCONSISTENCY_REPORT.md INC-037 (`form_fields` absente), INC-038 (angle mort du
--           signal de divergence), INC-039 (ordre de suppression d'un workspace)
--
...
--      source ;
--   6. les **écarts figés par des assertions** : `form_fields` n'existe pas (INC-037), une
--      suppression dans la source n'est pas détectée (INC-038), et la suppression d'un workspace
--      est refusée quand un workflow instancie ses nœuds (INC-039). Chacune deviendra rouge le jour
--      où l'état changera.
--
...
select columns_are(
	...
	array['workflow_id', 'workspace_id', 'name', 'track_id', 'derived_at', 'source_workflow_id',
	      'source_name', 'source_archived_at', 'source_modified_at', 'source_modified_since_copy'],
...
insert into public.workflow_nodes_catalog (id, workspace_id, key, label, position) values
	('c0b10000-0000-4000-8000-0000000000b1', 'c0b10000-0000-4000-8000-000000000001', 'un',   'Un',   1),
	('c0b10000-0000-4000-8000-0000000000b2', 'c0b10000-0000-4000-8000-000000000001', 'deux', 'Deux', 2),
	('c0b10000-0000-4000-8000-0000000000b3', 'c0b10000-0000-4000-8000-000000000001', 'trois','Trois',3);
...
--
-- `updated_at` est **antidaté d'un jour** sur toute la composition de la source, et ce n'est pas
...
select is(
	...
	'`source_modified_at` est bien le plus récent `updated_at` de la source ET de sa composition, '
	'et non celui de la seule ligne du workflow');
...
-- Remise en état, pour que l'assertion suivante porte sur un signal éteint.
update public.workflows set derived_at = now()
 where id = (select id from pg_temp_copie);

select ok(
	(select not source_modified_since_copy from public.workflow_derivations
	  where workflow_id = (select id from pg_temp_copie)),
	'une copie refaite après la modification n''affiche plus de divergence');

-- --- 7.1 L'ANGLE MORT, MESURÉ ET FIGÉ — INC-038 -------------------------------------------------
-- Cette assertion est l'inverse d'une assertion ordinaire : elle constate un DÉFAUT. Le jour où
-- INC-038 sera tranchée et le signal corrigé, elle deviendra rouge — et c'est ce qu'on lui demande.
...
select ok(
	(select not source_modified_since_copy from public.workflow_derivations
	  where workflow_id = (select id from pg_temp_copie)),
	'INC-038 : une SUPPRESSION dans la source ne modifie aucun `updated_at`, et la divergence '
	'reste invisible. Défaut mesuré, figé ici plutôt que confié à la mémoire — cette assertion '
	'devient rouge le jour où le signal est corrigé');
