...
--           (liaison CRM-018), INC-037 (la copie emporte les champs)
--
...
-- INC-037 refermée par le vrai geste de copie de CRM-018.
select is(
...
select is(
	(select count(*)::int from public.form_field_rules r
	  ...
	  where w.derived_from_workflow_id = '5eed0000-0000-4000-8000-000000000051'
	    and w.name = 'Cycle commercial — Conseil IA'),
	15, 'les quinze règles sont elles aussi remappées');

-- CRM-018 remplace l'ancien tableau par une liaison : le workflow global en porte une et sa copie
-- complète en porte une autre, remappée, ce qui rend INC-056 déterministe par construction.
select is(
	...
	1,
	'CRM-018 : UNE liaison de champ exigé existe sur le workflow GLOBAL');

select is(
	(select array_agg(trf.field_id order by trf.field_id)
	   from public.workflow_transition_required_fields trf
	  where trf.transition_id = '5eed0000-0000-4000-8000-000000000074'),
...
select is(
	(select count(*)::int from public.workflow_transition_required_fields trf
	  ...
	  where w.derived_from_workflow_id = '5eed0000-0000-4000-8000-000000000051'
	    and w.name = 'Cycle commercial — Conseil IA'),
	1, 'la copie porte exactement une exigence vers son propre champ dérivé');
