...
--           transition, toujours perdu), INC-014 (`profiles` illisible)
--
...
--   7. la **conformité du seed** : 27 événements, dont 4 avec un acteur réel ;
...
-- Les deux types que docs/SCHEMA.md §5 nomme et qu'AUCUN trigger n'écrit sont REFUSÉS. Le jour où
-- `CRM-054` et `CRM-058` les écriront, ces deux assertions deviendront rouges et désigneront la
-- contrainte à étendre — mécanisme de la décision 51.
select is(pg_temp.essayer_type(t), 'refuse',
	format('Le type `%s` est REFUSÉ tant qu''aucun trigger ne l''écrit (CRM-054, CRM-058)', t))
  from unnest(array['mail_received','mail_sent']) t;
...
select is(
	...
	0, 'Les neuf `created` n''ont AUCUN acteur : la clé de service ne porte pas de revendication '
