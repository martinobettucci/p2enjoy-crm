...
--   5. ce qui **reste dû** à `CRM-013`, figé par des assertions d'absence : cinq cibles sur six
--      portent sur des tables qui n'existent pas, et chaque assertion deviendra rouge le jour où
--      la table naîtra sans que la protection soit écrite (mécanisme de la décision 51).
--
...
-- =============================================================================================
-- 6. Ce qui reste dû à `CRM-013`, figé par des assertions d'absence
-- =============================================================================================
-- Cinq des six cibles de l'énoncé portent sur des tables qui n'existent pas. Chaque assertion
-- ci-dessous deviendra ROUGE le jour où la table naîtra — et c'est elle qui désignera le moment
-- d'écrire la protection correspondante (mécanisme de la décision 51).

select hasnt_table('public', 'mail_inbound_accounts',
	'CIBLE 1/6 NON LIVRÉE : `mail_inbound_accounts.secret_id` attend `CRM-052`. La preuve de refus '
	'n° 6 de `docs/SPEC-permissions-rls.md` §7 reste hors d''atteinte. CETTE ASSERTION DOIT DEVENIR '
	'ROUGE À `CRM-052`, et désigner alors le `REVOKE SELECT` à écrire');

select hasnt_table('public', 'mail_outbound_identities',
	'CIBLE 2/6 NON LIVRÉE : `mail_outbound_identities.secret_id` attend `CRM-053`, même refus n° 6');
