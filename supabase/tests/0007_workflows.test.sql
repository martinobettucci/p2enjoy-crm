...
--           INC-031 (garde d'archivage, moitié du chemin), INC-033 (corrigée par `CRM-018`)
...
--      `NOT NULL` de `channels.workflow_id` n'est pas posée (INC-029), et l'ancien `require_fields`
--      est remplacé par une relation (INC-033). Ces assertions ont rougi puis ont été révisées.
--
...
select columns_are(
	...
	array['id', 'workflow_id', 'workspace_id', 'from_step_id', 'to_step_id', 'label',
	      'require_comment', 'created_at', 'updated_at'],
...
select hasnt_column('public', 'workflow_transitions', 'require_fields',
	'CRM-018 : l''ancien tableau `require_fields` a disparu de la transition');
...
select has_table('public', 'workflow_transition_required_fields',
	'CRM-018 : les champs exigés vivent dans une table de liaison');
select col_is_pk('public', 'workflow_transition_required_fields',
	array['transition_id', 'field_id'], 'la liaison est unique par couple transition × champ');
...
-- CRM-018 ne change pas les trois clés propres à la transition : les deux nouvelles clés partent
-- de la table de liaison, là où PostgreSQL peut réellement les opposer.
select is(
	(select count(*)::int from pg_constraint
	  where conrelid = 'public.workflow_transitions'::regclass
	    and contype = 'f'),
	3,
	'CRM-018 : les transitions gardent exactement leurs trois clés — workflow et deux étapes ; les '
	'deux clés de champs partent de la nouvelle table de liaison');
...
-- constaté a changé trois fois : à `CRM-035` le tableau pouvait désigner un champ, à `CRM-036` la
-- garde le lisait, et `CRM-018` l'a remplacé par une relation.
select has_table('public', 'form_fields',
	'`form_fields` existe depuis `CRM-035` : l''assertion d''absence posée ici a été révisée, non '
	'retirée');

-- RÉVISÉE PAR `CRM-018`, NON RETIRÉE : le seed porte exactement une liaison effective sur le
-- workflow global et une liaison remappée sur sa copie complète.
select is(
	...
	1,
	'CRM-018 : UNE liaison effective existe sur le workflow GLOBAL du seed');

-- ET L'HÉRITAGE EST COMPTÉ PLUTÔT QUE SUBI. Sans cette assertion, la précédente serait verte
-- même si la copie perdait le comportement de sa source.
select is(
	(select count(*)::int from public.workflow_transition_required_fields trf
	   join public.workflow_transitions t on t.id = trf.transition_id
	...
	1,
	'INC-056 corrigée : la copie complète porte UNE liaison fonctionnelle remappée');
...
-- L'ancienne preuve acceptait un identifiant mort. Elle est renversée par `CRM-018`.
select throws_ok(
	$$insert into public.workflow_transition_required_fields (transition_id, field_id)
	  values ('88880000-0000-4000-8000-0000000000e1',
	          '99999999-9999-4999-8999-999999999999')$$,
	'23503', null,
	'INC-033 corrigée : une liaison vers un champ inexistant est refusée');
...
insert into public.profiles (id, full_name) values
	('99990000-0000-4000-8000-00000000000a', 'Admin WF A'),
	('99990000-0000-4000-8000-00000000000b', 'Viewer WF A'),
	('99990000-0000-4000-8000-00000000000c', 'Bizdev WF A'),
	('99990000-0000-4000-8000-00000000000d', 'Admin WF B');
