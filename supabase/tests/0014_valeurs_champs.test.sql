...
--           (`require_fields` sans intégrité), INC-047 (**close**), INC-053 (`user` non résolu),
...
-- imprévisible ; elle ne porte d'ailleurs aucun champ (INC-037).
--
...
-- --- 6.5 L'UNION : `require_fields` de la transition exige, indépendamment de l'étape ----------
...
select is(
	...
	'lien-proposition',
	'L''UNION du §3.5 : le refus vient du `require_fields` de la TRANSITION, l''étape cible '
...
-- --- 6.7 Un identifiant de `require_fields` non résolu est IGNORÉ — décision 128 ---------------
-- Dénoncer bloquerait définitivement une transition pour une erreur de saisie d'administrateur,
-- qu'aucun utilisateur ne peut corriger depuis le produit.
select pg_temp.postgres();
...
select lives_ok($$
	select public.move_card('5eed0000-0000-4000-8000-0000000000c7',
	                        '5eed0000-0000-4000-8000-000000000066') $$,
	'INC-033, décision 128 : un identifiant de `require_fields` que la jointure NE RÉSOUT PAS est '
	'IGNORÉ, non dénoncé. Le comportement est FIGÉ ici plutôt que laissé au hasard de l''écriture');

-- --- 6.8 Un champ `hidden` MAIS désigné par `require_fields` EST exigé -------------------------
...
update public.workflow_transitions
   set require_fields = array['5eed0000-0000-4000-8000-000000000084'::uuid]   -- motif-perte
 where id = '5eed0000-0000-4000-8000-000000000072';                            -- relance → négociation
...
select is(
	pg_temp.detail_du_refus('5eed0000-0000-4000-8000-0000000000c2',
	                        '5eed0000-0000-4000-8000-000000000063'),
	'motif-perte',
	'un champ `hidden` à l''étape cible MAIS désigné par `require_fields` EST exigé : l''union du '
...
select is(
	(select count(*)::int from public.card_field_values),
	0,
	'PREUVE N° 11 : l''anonyme lit ZÉRO ligne — pas une erreur. Le seed en pose pourtant quatorze');
