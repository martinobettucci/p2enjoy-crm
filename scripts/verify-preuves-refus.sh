...
# @verifies docs/INCONSISTENCY_REPORT.md INC-014 (tables d'identité), INC-055 (un harnais ne
#           rejoue pas un préfixe de migrations), INC-057 (le `@verifies` de `cards.spec.ts`)
# @verifies docs/JOURNAL.md décisions 146 à 150
#
# Rejoue les preuves exigées par la Definition of Done de `CRM-014` :
#
#   1. la suite pgTAP `supabase/tests/0016_preuves_refus.test.sql` est verte — 46 assertions ;
#   2. le fichier consolidé `e2e/api/preuves-refus.spec.ts` est vert — 37 scénarios, avec les
...
#      - `cards_lecture` **retirée** doit faire échouer les scénarios ET la suite pgTAP ;
...
# est anonyme, et un refus opposé à un profil authentifié est par construction hors de sa portée.
#
...
ASSERTIONS_ATTENDUES=46
SCENARIOS_ATTENDUS=37
...
nb_politiques=$(printf '%s\n' "$REFERENCE" | grep -c . || true)
if [ "$nb_politiques" = "41" ]; then
	ok "41 politiques relevées dans le schéma public, conformes à ce que la suite pgTAP assère"
else
	fail "41 politiques attendues, $nb_politiques relevées"
fi
...
if pgtap_vert "$sortie_tap_permissive"; then
	fail "la suite pgTAP reste VERTE avec une politique de plus — le compte de 41 ne sert à rien"
else
...
cards_seed=$(psql_db -c "select count(*) from public.cards;")
if [ "$cards_seed" = "9" ]; then
	ok "les neuf cards du seed sont intactes"
else
	fail "9 cards attendues dans le seed, $cards_seed trouvées"
fi

ws_seed=$(psql_db -c "select count(*) from public.workspaces;")
if [ "$ws_seed" = "1" ]; then
	ok "le seed compte toujours un seul workspace"
else
	fail "1 workspace attendu, $ws_seed trouvés"
fi
