...
# @verifies docs/SPEC-cards.md §14.2 (modèle), §14.3 (`clock_timestamp()`), §14.4 (les dix
...
#      `clock_timestamp()`, le `CHECK` des dix types ;
...
# ---------------------------------------------------------------------------------------------
# Le compte exact de 41 événements ne vaut qu'IMMÉDIATEMENT APRÈS l'application du seed sur une
...
# et le seul compte exact asséré est celui des quatorze naissances — une card ne naît qu'une fois.
#
...
MIGRATION_ACTUELLE=supabase/migrations/0020_change_channel_workflow.sql
...
CARDS_SEED="'5eed0000-0000-4000-8000-0000000000c1','5eed0000-0000-4000-8000-0000000000c2',
	...
	'5eed0000-0000-4000-8000-0000000000c9','5eed0000-0000-4000-8000-0000000000ca',
	'5eed0000-0000-4000-8000-0000000000cb','5eed0000-0000-4000-8000-0000000000cc',
	'5eed0000-0000-4000-8000-0000000000cd','5eed0000-0000-4000-8000-0000000000ce'"

naissances=$(psql_db -c "select count(*) from public.card_events where type='created' and card_id in ($CARDS_SEED)")
if [ "$naissances" = 14 ]; then
	ok "QUATORZE naissances, une par card du seed — le seul compte exact qui tienne (décision 210)"
else
	fail "le seed ne porte pas quatorze événements « created » mais $naissances"
fi
...
for couple in "moved:2" "assigned:2" "channel_changed:2" "field_changed:21"; do
...
# Puis la 17, propriétaire historique de `channel_changed`, avant la 20 qui est désormais la
# dernière autorité sur le trigger et le vocabulaire (INC-074, décisions 219 et 306).
if docker exec -i "$DB_CONTAINER" psql -U postgres -d postgres -v ON_ERROR_STOP=1 \
...
if docker exec -i "$DB_CONTAINER" psql -U postgres -d postgres -v ON_ERROR_STOP=1 \
	...
	ok "la migration 20 est rejouée en dernier : workflow_changed et l'exclusivité survivent"
else
	fail "le rejeu de la migration 20 ÉCHOUE : voir $TRAVAIL/rejeu20.log"
fi

if [ "$(psql_db -c "select prosrc like '%channel_changed%'
	and prosrc like '%workflow_changed%' from pg_proc p
	join pg_namespace n on n.oid = p.pronamespace
	where n.nspname='app' and p.proname='card_events_apres_maj_card'")" = t ]; then
	ok "les gardes channel_changed et workflow_changed sont rendues — constatées, non supposées"
else
	fail "le trigger est resté dans une forme historique : la restauration a régressé"
fi
