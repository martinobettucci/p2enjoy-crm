...
# @verifies docs/INCONSISTENCY_REPORT.md INC-021 (webapp anonyme), INC-014 (profils illisibles),
#           INC-048 (le motif n'est conservé nulle part), INC-066 (aucun éditeur de workflow)
#
...
#   5. le seed porte ce que le board démontre : sept étapes, dix transitions dont quatre à motif,
#      deux étapes sans transition sortante, trois cards actives sur deux étapes, une archivée et
#      une en corbeille ;
...
if [ "${transitions:-0}" -eq 10 ]; then
	ok "il porte ses dix transitions : le menu du §7.5 a de quoi être non vide"
else
	fail "il porte $transitions transitions au lieu de 10"
fi

a_motif=$(psql_db -c "select count(*) from public.workflow_transitions where workflow_id='$WORKFLOW_GLOBAL' and require_comment")
if [ "${a_motif:-0}" -eq 4 ]; then
	ok "quatre transitions exigent un motif : le §7.8 est démontrable en permanence"
else
	fail "$a_motif transitions exigent un motif au lieu de 4 : le §7.8 n'est plus démontré"
fi
...
if [ "${actives:-0}" -eq 3 ]; then
	ok "grands-comptes porte trois cards actives"
else
	fail "grands-comptes porte $actives cards actives au lieu de 3"
fi
...
if [ "${occupees:-0}" -eq 2 ]; then
	ok "elles n'occupent que deux étapes sur sept : cinq colonnes vides sont démontrées"
else
	fail "les cards occupent $occupees étapes au lieu de 2"
fi
