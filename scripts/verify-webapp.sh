...
# C'est le contrôle décisif : la ligne existe en base, et **aucun** appelant de l'API ne la
# voit — ni anonyme, ni authentifié. L'écran vide de la webapp est donc exact.
if [ "$reponse_auth" = "200" ] && [ "$corps_auth" = "[]" ] && [ "$lignes_en_base" -ge 1 ]; then
	ok "jeton réel : 200 et [] alors que la base contient $lignes_en_base ligne(s) — l'écran vide est exact"
else
	fail "jeton réel : attendu 200 et [] avec des lignes en base ; obtenu $reponse_auth, $corps_auth, $lignes_en_base ligne(s)"
fi
