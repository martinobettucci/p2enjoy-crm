...
#           §9.3 (les cinq cards), §9.4 (identifiants résolus), §9.5 (copie du formulaire),
...
# ---------------------------------------------------------------------------------------------
# Il ne prouve **aucun écran** : ce harnais reste volontairement une preuve de données et d'API
# avec les jetons réels. Les parcours connectés au clavier et à la souris appartiennent aux suites
# d'interface dédiées ; la formulation exacte du contrat vérifié ici est au §9.7.
#
...
COMMENTAIRES_SEED_SQL="'5eed0000-0000-4000-8000-0000000000d1',
	...
	'5eed0000-0000-4000-8000-0000000000d4',
	'5eed0000-0000-4000-8000-0000000000d5'"
CHANNELS_SEED_SQL="'5eed0000-0000-4000-8000-000000000031',
	...
	'5eed0000-0000-4000-8000-000000000036'"
...
# L'empreinte du §9.8.
# ---------------------------------------------------------------------------------------------
# Elle porte tout ce que la FIXTURE OFFICIELLE doit rendre identique d'une reconstruction à l'autre,
# jamais les données utilisateur que le seed doit préserver (décision 300). Chaque famille est donc
# bornée par ses identifiants stables. Sont aussi exclus : l'identifiant et
...
COPIE_ID=$(psql_db -c "select id from workflows
                        where derived_from_workflow_id = '$WF_GLOBAL'
                          and track_id = '$TRACK_CONSEIL'
...
# --- 6. Le formulaire du dérivé est une copie complète et autonome -----------------------------
...
[ "$regles_derive" = '15' ] \
	&& ok "les quinze règles sont remappées vers les étapes et champs de la copie" \
	|| fail "le workflow dérivé porte $regles_derive règle(s), quinze attendues"

exigences_derive=$(psql_db -c "select count(*) from workflow_transition_required_fields rf
	join workflow_transitions t on t.id=rf.transition_id where t.workflow_id='$COPIE_ID'")
[ "$exigences_derive" = '1' ] \
	&& ok "l'exigence de transition est remappée vers le champ de la copie" \
	|| fail "le workflow dérivé porte $exigences_derive exigence(s), une attendue"

champs_partages=$(psql_db -c "select count(*) from form_fields source
	join form_fields copie on copie.id=source.id
	where source.workflow_id='$WF_GLOBAL' and copie.workflow_id='$COPIE_ID'")
[ "$champs_partages" = '0' ] \
	&& ok "aucun identifiant de champ n'est partagé avec la source" \
	|| fail "$champs_partages identifiant(s) de champ partagé(s) avec la source"

valeurs_derive=$(psql_db -c "select count(*) from card_field_values
	where workflow_id = '$COPIE_ID' and card_id in ('$CARD_CA', '$CARD_CB')")
if [ "$valeurs_derive" = '3' ]; then
	ok "les deux cards dérivées portent trois valeurs par leurs champs remappés"
else
	fail "$valeurs_derive valeur(s) sur le workflow dérivé, trois attendues"
fi

# --- 7. Les sept valeurs ajoutées -------------------------------------------------------------

titre "7. Preuve n° 7 — les sept valeurs du §9.6"
...
verifier_valeur "$CARD_CC" "$CHAMP_LIEN" '…0cc lien-proposition, exigé par la liaison de l’arête'
...
verifier_valeur_derivee() {
	local card=$1 cle=$2 libelle=$3
	local v
	v=$(psql_db -c "select v.value::text from card_field_values v
		join form_fields f on f.id=v.field_id
		where v.card_id='$card' and v.workflow_id='$COPIE_ID' and f.key='$cle'")
	if [ -n "$v" ] && [ "$v" != 'null' ]; then
		ok "$libelle : renseignée ($v)"
	else
		fail "$libelle : absente ou vide (« ${v:-aucune ligne} »)"
...
verifier_valeur_derivee "$CARD_CA" source '…0ca source de la card dérivée'
verifier_valeur_derivee "$CARD_CB" budget '…0cb budget de la négociation dérivée'
verifier_valeur_derivee "$CARD_CB" lien-proposition '…0cb proposition de la négociation dérivée'
...
# décision 210. Le §9.6 annonce 41 événements ; c'est vrai **au sortir d'un seed sur base neuve**,
...
#
# Ce qui est STABLE, et qui est donc assert : un `created` par card seedée, exactement, puisqu'une
...
verifier_minorant 'événements seedés, tous types'  41 "select count(*) from card_events where card_id in ($CARDS_SEED_SQL)"
verifier_minorant 'événements seedés field_changed' 21 "select count(*) from card_events where card_id in ($CARDS_SEED_SQL) and type = 'field_changed'"
verifier_minorant 'événements seedés moved'          2 "select count(*) from card_events where card_id in ($CARDS_SEED_SQL) and type = 'moved'"
verifier_minorant 'événements seedés assigned'       2 "select count(*) from card_events where card_id in ($CARDS_SEED_SQL) and type = 'assigned'"
