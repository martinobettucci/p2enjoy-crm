...
# @verifies docs/INCONSISTENCY_REPORT.md INC-037 (formulaire remappé), INC-038 (suppression visible),
...
# Le ménage retire uniquement les copies créées par CE harnais, identifiables par son préfixe
# réservé. Une copie utilisateur sur un autre track est une donnée produit, jamais un déchet de
# test (décision 300). Supprimer le workflow témoin suffit, ses enfants tombant en cascade.
menage() {
	psql_db -c "
		...
		select string_agg(x, '|' order by x) from (
			select 'fn:' || p.proname || ':' || pg_get_function_identity_arguments(p.oid)
			 ...
			 where (n.nspname = 'public' and p.proname = 'copy_workflow_to_track')
			    or (n.nspname = 'app' and p.proname = 'workflow_composition_fingerprint')
			union all
			...
			union all
			select 'column:' || a.attname || ':' || a.atttypid::regtype::text
			  from pg_attribute a
...
[ "$mesure" = "7/11/1/1,2,3,4,5,6,7/false/track" ] \
	&& ok "sept étapes, dix arêtes, une initiale, positions conservées, \`is_default\` forcé, portée \`track\`" \
	|| fail "contenu de la copie : « $mesure », attendu « 7/11/1/1,2,3,4,5,6,7/false/track »"
...
copies_studio_avant=$(psql_db -c "select count(*) from public.workflows
                                  where derived_from_workflow_id = '$WF_SEED'
                                    and track_id = '$TRACK_STUDIO';")
code=$(http POST "$API/rest/v1/rpc/copy_workflow_to_track" -H "apikey: $ANON_KEY" \
...
if [ "$code" = "403" ] && [ "$message" = "forbidden" ] \
	&& [ "$reste" = "$copies_studio_avant" ]; then
...
contenu=$(psql_db -c "
	select (select count(*) from public.workflow_steps s
	         where s.workflow_id = (select id from public.workflows
	                                 where derived_from_workflow_id = '$WF_SEED'
	                                   and name = '$NOM_COPIE_SEED'))::text
	    || '/' || (select count(*) from public.workflow_transitions t
	                where t.workflow_id = (select id from public.workflows
	                                        where derived_from_workflow_id = '$WF_SEED'
	                                          and name = '$NOM_COPIE_SEED'))::text
	    || '/' || (select count(*) from public.form_fields f
	                where f.workflow_id = (select id from public.workflows
	                                        where derived_from_workflow_id = '$WF_SEED'
	                                          and name = '$NOM_COPIE_SEED'))::text
	    || '/' || (select count(*) from public.form_field_rules r
	                where r.workflow_id = (select id from public.workflows
	                                        where derived_from_workflow_id = '$WF_SEED'
	                                          and name = '$NOM_COPIE_SEED'))::text
...
titre "6. INC-037 et INC-038 : formulaire complet et suppression détectée"

# CONTRÔLE RÉVISÉ PAR `CRM-018`, NON RETIRÉ : la mesure historique attendait zéro champ dérivé ;
# elle exige désormais l'équivalence métier et laisse les identifiants propres à chaque workflow.
...
code=$(http POST "$API/rest/v1/rpc/copy_workflow_to_track" -H "apikey: $ANON_KEY" \
	-H "Authorization: Bearer $T_ADMIN" -H 'Content-Type: application/json' \
	-d "{\"workflow_id\":\"$WF_SEED\",\"track_id\":\"$TRACK_STUDIO\",\"new_name\":\"tst-crm032-suppression\"}")
COPIE_SUPPRESSION=$(jq -r 'if type == "string" then . else empty end' < "$CORPS")
if [ "$code" = "200" ] && [ -n "$COPIE_SUPPRESSION" ]; then
	ok "création de la copie témoin de suppression"
	psql_db -c "delete from public.workflow_transitions
	             where id = 'c0b1f000-0000-4000-8000-00000000000f';" >/dev/null
	angle=$(psql_db -c "select source_modified_since_copy from public.workflow_derivations
	                     where workflow_id = '$COPIE_SUPPRESSION';")
	[ "$angle" = "t" ] \
		&& ok "INC-038 : supprimer l'arête source allume la divergence par empreinte" \
		|| fail "INC-038 : le signal après suppression vaut « $angle », attendu t"
	psql_db -c "delete from public.workflows where id = '$COPIE_SUPPRESSION';" >/dev/null
else
	fail "création de la copie témoin de suppression : code $code"
	psql_db -c "delete from public.workflow_transitions
	             where id = 'c0b1f000-0000-4000-8000-00000000000f';" >/dev/null
fi
