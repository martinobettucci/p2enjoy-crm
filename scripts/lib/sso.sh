...
# mot de passe publié du realm de développement, docs/SPEC-session-sso.md §10).
#
...
# Rend 0 et le JSON du point de jeton sur la sortie standard, ou 1 et un diagnostic sur stderr.
sso_connexion_pkce() {
	local adresse=$1
	...
	local decouverte autorisation point_jeton verificateur defi entetes page cookies action location code
	...
	point_jeton=$(jq -r .token_endpoint <<<"$decouverte")
	...
	curl -sf --max-time 10 "$point_jeton" \
