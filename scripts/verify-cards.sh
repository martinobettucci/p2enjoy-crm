...
menage() {
	psql_db -c "delete from public.cards where title like 'tst-crm040-%';" >/dev/null 2>&1 || true
}
...
# « Active » a une définition, et elle compte : le nœud `livre` ne porte que la card ARCHIVÉE.
noeud_libre=$(psql_db -c "select id from public.workflow_nodes_catalog
                           where workspace_id = '$WS_SEED' and key = 'livre';")
...
psql_db -c "update public.workflow_nodes_catalog set archived_at = null where id = '$noeud_libre';" \
	>/dev/null
...
[ "$etat" = "9/1/1/9" ] \
	&& ok "neuf cards, une archivée, une en corbeille, neuf adresses distinctes" \
	|| fail "état du seed : « $etat », attendu « 9/1/1/9 »"

[ "$(psql_db -c "select count(*) from public.cards c
                  join public.channels ch on ch.id = c.channel_id
                 where ch.slug = 'prospection';")" = "0" ] \
	&& ok "aucune card dans \`prospection\` : INC-046, motif écrit en docs/SPEC-cards.md §9.1" \
	|| fail "une card réside dans \`prospection\` — le seed ne peut plus converger"
