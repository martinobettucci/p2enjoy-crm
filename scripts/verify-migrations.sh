...
#   4. le refus par défaut est réel, mesuré **hors interface** avec les jetons réels : lecture à
#      zéro ligne pour l'anonyme comme pour un compte authentifié, écriture refusée, schéma `app`
#      injoignable par l'API ;
...
docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d migrations-runner \
	>/tmp/p2enjoy-runner.log 2>&1 || true
runner_code=$(docker inspect -f '{{.State.ExitCode}}' p2enjoy-migrations 2>/dev/null || echo -1)
if [ "$runner_code" = 0 ]; then
	ok "le conteneur migrations-runner rejoue le répertoire et se termine avec le code 0"
else
...
# --- 4. Refus par défaut, mesuré hors interface ------------------------------------------------
# `docs/SPEC-permissions-rls.md` §7 : un refus de lecture se manifeste par **zéro ligne**, jamais
# par une erreur ambiguë. Les deux formes sont mesurées séparément.

echo
echo "4. Refus par défaut (RLS activée, aucune politique jusqu'à CRM-010)"
...
if [ "$code" = 200 ] && [ -n "$JETON" ]; then
	ok "connexion du compte de preuve : jeton d'accès obtenu"
else
	fail "connexion refusée (HTTP $code) : $(http_body | head -c 200)"
fi

if [ -n "${JETON:-}" ]; then
	code=$(http GET "$API/rest/v1/profiles?select=*" \
		-H "apikey: $ANON_KEY" -H "Authorization: Bearer $JETON")
	corps=$(http_body)
	if [ "$code" = 200 ] && [ "$corps" = "[]" ]; then
		ok "compte authentifié : son propre profil reste invisible tant qu'aucune politique n'existe"
	else
		fail "lecture par un compte authentifié : HTTP $code, corps $(printf '%s' "$corps" | head -c 120)"
	fi
	...
	code=$(http PATCH "$API/rest/v1/profiles?id=eq.$USER_ID" \
		-H "apikey: $ANON_KEY" -H "Authorization: Bearer $JETON" \
		-H 'Content-Type: application/json' -H 'Prefer: return=representation' \
		-d '{"full_name":"Usurpation"}')
	corps=$(http_body)
	if [ "$corps" = "[]" ] || [ "$code" -ge 400 ]; then
		ok "compte authentifié : modification de son propre profil sans effet (HTTP $code)"
	else
		fail "une modification de profil a été appliquée sans politique (HTTP $code)"
	fi
fi
...
# 4.4 Suppression du compte : la cascade emporte le profil.
curl -s -o /dev/null -X DELETE "$API/auth/v1/admin/users/$USER_ID" \
	-H "apikey: $SERVICE_ROLE_KEY" -H "Authorization: Bearer $SERVICE_ROLE_KEY"
code=$(http GET "$API/rest/v1/profiles?id=eq.$USER_ID&select=id" \
	-H "apikey: $SERVICE_ROLE_KEY" -H "Authorization: Bearer $SERVICE_ROLE_KEY")
if [ "$code" = 200 ] && [ "$(http_body)" = "[]" ]; then
	ok "la suppression du compte par GoTrue supprime le profil"
else
	fail "profil encore présent après suppression du compte (HTTP $code) : $(http_body | head -c 120)"
fi
USER_ID=""
