...
# @verifies docs/INCONSISTENCY_REPORT.md INC-025 (colonnes communes), INC-033 (`require_fields`),
#           INC-037 (la copie n'emporte pas les champs), INC-043 (`CRM-034` sans cible)
#
...
#   6. l'écart d'INC-037 est **compté** : la source porte sept champs, la copie zéro ;
...
ETAPE_COPIE=$(psql_db -c "select s.id from public.workflow_steps s
                           ...
                           where w.derived_from_workflow_id = '$WF_GLOBAL' limit 1;")
...
titre "6. INC-037 : l'écart est compté, pas corrigé"
...
[ "$champs_source" = "7" ] && [ "$champs_copie" = "0" ] \
	&& ok "la source porte 7 champs, la copie **0** — \`copy_workflow_to_track\` n'en copie aucun, "\
