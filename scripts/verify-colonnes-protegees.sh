...
#
# Il ne prouve **ni la preuve de refus n° 6 ni la n° 8** de docs/SPEC-permissions-rls.md §7 :
# `mail_inbound_accounts`, `mail_outbound_identities`, `api_tokens`, `card_events` et `audit_log`
# n'existent pas. La section 7 le CONSTATE table par table plutôt que de l'affirmer.
#
...
# l'identique par le harnais suivant. La séquence de restauration est donc 12 → 13 → 14, jamais
# partielle.
...
psql_db -v ON_ERROR_STOP=1 -f - < "$MIGRATION_FILE" >/dev/null 2>&1 || true
[ "$(prive authenticated email_local_part update)" = "f" ] \
	&& ok "…et la 14 rejouée derrière elle la referme : l'ordre 12 → 13 → 14 restaure tout" \
...
[ "$(psql_db -c "select count(*) from public.cards;")" = "9" ] \
	&& ok "les neuf cards du seed sont intactes" \
	|| fail "le nombre de cards a changé : une sonde n'a pas été nettoyée"
[ "$(psql_db -c "select count(distinct email_local_part) from public.cards;")" = "9" ] \
	&& ok "neuf adresses DISTINCTES : l'index unique tient" \
	|| fail "des adresses se répètent"
[ "$(psql_db -c "select count(*) from public.cards
                  where email_local_part ~ '^c-[0-9abcdefghjkmnpqrstvwxyz]{8}\$';")" = "9" ] \
	&& ok "…et toutes ont la forme générée : aucune n'a été réécrite par ce harnais" \
	|| fail "au moins une adresse n'a pas la forme générée"

titre "7. Ce qui reste dû à CRM-013, compté table par table"

for table in mail_inbound_accounts mail_outbound_identities api_tokens card_events audit_log; do
