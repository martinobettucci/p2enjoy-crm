...
#           INC-067 (représentation de `cards.amount`)
#
...
#   5. le seed porte ce que la liste démontre : trois cards actives sur deux étapes, sept étapes,
...
# pas de channel de plus de trois cards actives. La seconde page est prouvée contre une réponse
...
if [ "${actives:-0}" -eq 3 ]; then
	ok "trois cards actives dans « Grands comptes » : le tableau a de quoi être non vide"
else
	fail "$actives cards actives au lieu de 3 : les preuves de la liste ne tiennent plus"
fi
...
# Le nom du responsable n'est lisible par personne (INC-014) : la colonne n'est pas demandée.
# Le contrôle porte sur la **déclaration**, non sur le fichier : un commentaire qui explique
# pourquoi `owner_id` est absente ne doit pas faire échouer le contrôle qui l'exige absente.
colonnes_demandees=$(grep -A2 "^export const COLONNES_CARD_LISTE" "$COLONNES" | tr -d '\n')
for absente in owner_id position description health_score; do
	if ! printf '%s' "$colonnes_demandees" | grep -q "$absente"; then
		ok "$absente n'est pas demandée par la page (§12.3)"
	else
		fail "$absente est demandée alors que rien ne l'affiche (INC-014, §12.3)"
	fi
done
