...
#           INC-031 (garde d'archivage : moitié du chemin), INC-033 (liaison CRM-018)
#
...
exigences=$(psql_db -c "select count(*) from public.workflow_transition_required_fields trf
                         join public.workflow_transitions t on t.id = trf.transition_id
                        where t.workflow_id = '$WF_SEED';")
...
[ "$avec_commentaire" = "5" ] \
	&& ok "cinq transitions exigent un commentaire — celles qui mènent à « Perdu »" \
	|| fail "transitions exigeant un commentaire : $avec_commentaire, attendu 5"
...
[ "$exigences" = "1" ] \
	&& ok "CRM-018 : une liaison de champ exigé sur le workflow global, avec intégrité référentielle" \
	|| fail "liaisons de champs exigés : $exigences, attendu 1"
...
# `Réalisation → Perdu` EST déclarée depuis la décision 259 (INC-003, close). Ce contrôle figeait
# l'absence — « le point ouvert n° 1 est tenu » —, et c'est précisément ce qui rendait l'oubli
# durable : une absence prouvée ne se distingue plus d'une règle. Il vérifie désormais la présence,
# ET l'exigence de commentaire qui l'accompagne, comme les quatre autres sorties vers Perdu.
realisation_perdu=$(psql_db -c "
	select count(*) from public.workflow_transitions t
	 ...
	 where t.workflow_id = '$WF_SEED' and nd.key = 'realisation' and na.key = 'perdu'
	   and t.require_comment;")
[ "$realisation_perdu" = "1" ] \
	&& ok "\`Réalisation → Perdu\` est déclarée et exige un motif : INC-003 close" \
	|| fail "\`Réalisation → Perdu\` manque ou n'exige pas de motif : le cul-de-sac est revenu"
...
[ "$restant" = "10" ] \
	...
	|| fail "dégradation c : $restant transitions, attendu 10"
...
")
if [ "${restaure%%/*}" = "1" ] \
	&& [ "$(printf '%s' "$restaure" | cut -d/ -f2)" = "11" ] \
	&& printf '%s' "$restaure" | grep -q 'is_workspace_admin' \
	&& printf '%s' "$restaure" | grep -q 'FOREIGN KEY (to_step_id, workflow_id)'; then
	ok "restauration constatée : index revenu, onze transitions revenues, politique revenue à "\
