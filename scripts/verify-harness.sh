...
#
# **RÉVISÉS À 55 ET 2651 PAR `CRM-063` SOUS-TRANCHE 2b, le 2026-08-25.** Elle livre
# `supabase/tests/0055_guichet_variables_modeles.test.sql` — la forme du guichet des variables, ses
# privilèges rôle par rôle, et l'ÉGALITÉ avec la fonction déléguée : `FICHIERS_SQL_ATTENDUS` passe
# de 54 à **55**, et 2640 + 11 = **2651**. Elle ne RÉVISE aucune suite existante : la migration 57
# n'ajoute ni table, ni colonne, ni politique, et ne fait que déléguer.
#
# LA VALEUR EST COMPTÉE PAR LES DEUX CHEMINS, comme celle de 2a. `npm run test:sql` somme
# **55 fichiers, 2651 assertions, aucune anomalie** — cette exécution-ci a vu
...
# **73 depuis `CRM-092` tranche T8, 2026-09-24** (décision 597) : `0073_admin_du_domaine.test.sql`,
# seul fichier ajouté. 72 + 1 = 73, valeur COMPTÉE — « 73 fichiers ».
FICHIERS_SQL_ATTENDUS=73
...
# **RÉVISÉE à 885 par `CRM-063` sous-tranche 2b, le 2026-08-25** :
# `e2e/api/guichet-variables-modeles.spec.ts` est neuf et déclare **4** scénarios pour les quatre
...
#
# **1021 depuis `CRM-083` tranche 2 h, 2026-08-28.** La reprise d'un tableau archivé ajoute CINQ
# scénarios à `e2e/api/objectifs.spec.ts` — les mesures qui fondent son contrat, avec les jetons
# réels des trois profils. Le sixième point d'écart est ANTÉRIEUR et étranger à cette tranche : le
...
# **1082 depuis `CRM-092` tranche T8** : `session.spec.ts` gagne les deux scénarios de la règle du
# domaine sur `admin` — l'exploitante admise et administratrice sans ligne, puis le rôle retiré qui
# ferme l'accès. 1080 + 2 = 1082, valeur COMPTÉE — « Total: 1082 tests in 67 files ».
SCENARIOS_API=1082
...
# **`CRM-063` sous-tranche 2b, le 2026-08-25** : `e2e/ui/reglages-modeles-emails.spec.ts` est neuf
# et déclare **10** scénarios — la place de l'écran dans l'index, la création avec insertion d'une
# variable de la palette, le refus d'une variable inconnue traduit, la prévisualisation et ses trous
# nommés, la suppression confirmée, le zéro-ligne de la lectrice, et les **quatre paliers**, chacun
...
#
# **696 depuis `CRM-083` tranche 2 h, 2026-08-28.** La reprise d'un tableau archivé ajoute CINQ
...
# **729 depuis `CRM-081` tranche 3, 2026-08-29, et l'écart est ENTIÈREMENT le mien.** Les deux
# écarts nommés de l'unité ajoutent DEUX scénarios, dans deux fichiers existants et AUCUN fichier
# neuf : `e2e/ui/filtre-sommeil.spec.ts` éprouve enfin le §16.12.6 SANS aucun autre filtre — liste
# et board —, et `e2e/ui/sommeil-fil.spec.ts` éprouve le mode d'affichage qui survit au
