...
seedées sans commentaire utilisateur ni activité dans leur timeline. Une timeline absente — cas
d'une mise à niveau sans backfill — ou son unique `created` technique sans acteur est admise ; tout
autre événement prouve un geste à conserver et arrête le seed avant la première suppression
(décision 305). Toute copie supplémentaire est conservée ; toute ambiguïté arrête le seed. Une
copie utilisateur n'est jamais écrasée automatiquement.
