...
| 2.c | `CRM-005` → `CRM-007`, puis **`CRM-009`**, puis `CRM-008` | Seed, types et coquille, puis l'**écran de connexion**, puis le harnais de tests. `CRM-009` s'insère **entre `CRM-007` et `CRM-008`** par arbitrage du responsable (`docs/JOURNAL.md`, décision 253, INC-021) : la coquille existe avant l'écran, et le harnais vient après ce qu'il doit exercer. La DoD de `CRM-008` est bornée aux commandes dont le sujet existe ; pytest naît avec `mail-sync` en `CRM-051` (décision 277, INC-023), sans retenir le chunk 2 ouvert jusque-là. |
| 2.d | **`CRM-015` → `CRM-019`** | Cinq unités créées par arbitrage du responsable, toutes issues de contradictions mesurées : secret de build du registre npm (`CRM-015`, décision 255), fonctions edge (`CRM-016`, décision 260), ordonnancement par `pg_cron` (`CRM-017`, décision 261), `require_fields` en table de liaison (`CRM-018`, décision 262), `change_channel_workflow` (`CRM-019`, décision 263). `CRM-015` conditionne toute preuve exigeant `./runDev.sh` de bout en bout ; `CRM-018` **rouvre `CRM-031` et `CRM-035`**, livrées et prouvées. |
| 2.e | **`CRM-022`** | Ferme les lectures d'identité et les mutations de membership avant de reprendre les écrans qui doivent afficher de vraies personnes ; unité créée par la décision 294. |
...
| Production sur la cellule Spark | `docs/SPEC-deploiement-spark.md` |
| Authentification, SSO | `docs/SPEC-auth.md` (§10 pour le SSO), `docs/SSO.md` |
