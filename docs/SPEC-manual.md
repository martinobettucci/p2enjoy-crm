...
| 7 | sommaire ch. 24 | « quatorze réponses sur six affaires » | **18 réponses sur 9 affaires** |
...
| 12 | ch. 7 *bis* | « laisse une trace "changement de dossier" dans l'historique » | `card_events` porte **2 lignes `channel_changed`**, et `PanneauTimeline.tsx` ne déclare **aucun libellé** pour ce type : le fil affiche « Événement ». **Écart du PRODUIT — INC-077, non résolue ici** |
...
**Le douzième écart ne se corrige pas dans cette unité.** Ajouter un libellé au fil serait modifier
un écran depuis une unité documentaire, contre `CLAUDE.md` §1. Le manuel dira donc ce que le fil
**montre réellement** — un fait sans nom — et INC-077 portera l'arbitrage.
...
servi par le réseau, pour **mesurer** — et non déduire — ce que le fil en montre. C'est la preuve
d'INC-077, et la seule du dépôt qui rende ce type visible.
