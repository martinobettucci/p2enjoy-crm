...
| 9 | Téléchargement d'une pièce jointe `infected` ou `pending` | Refus — **ACQUISE par `CRM-054`** : le bucket `mail-attachments` est **privé** et `storage.objects` ne porte **aucune** politique de lecture. Mesuré avec les deux jetons — anonyme et administratrice — sur une pièce `infected` **et** une pièce `pending`. Attention pour `CRM-057` : `storage.objects` accorde tous les privilèges à `anon` et `authenticated`, et seule l'absence de politique refuse ; la politique de téléchargement devra donc être conditionnée à `av_status = 'clean'` |
