...
| `/contacts/:idContact` | **Fiche d'un contact** — `CRM-060` tranche 4f, `docs/SPEC-contacts.md` §15.2. Une route de DÉTAIL sous le carnet, pour le motif exact de la précédente. **Aucune collision avec elle**, et ce n'est pas une chance mais une propriété du chemin : ce patron porte DEUX segments là où celui de l'organisation en porte TROIS, et aucune adresse ne peut satisfaire les deux. Hors de `ROUTES` — son titre est le nom du contact, donc une donnée. Le contact est désigné par son identifiant : `contacts` ne porte aucun slug, et l'email est nul pour un contact du seed |
| `/couts` | **Cumul des coûts du workspace** — `CRM-086` tranche 5, `docs/SPEC-costs.md` §4.0 et §4.5. Un groupe de barres par track, cumulant ses budgets ouverts, avec un histogramme par devise présente. Une route de premier niveau portée par une entrée de la barre latérale, pour le motif exact du carnet : un histogramme de coûts n'administre rien, il porte le travail. **Seule des trois adresses de coûts à figurer dans `ROUTES`** — son titre est une clé de traduction et son contenu ne dépend d'aucun paramètre d'adresse |
...
| `/ma-journee` | **Ma journée** — `CRM-061`, `docs/SPEC-cards.md` §17. Les affaires actives portant une échéance, rangées en trois sections — En retard, Aujourd'hui, À venir dans sept jours — à travers tout l'espace de travail. Une route de premier niveau portée par une entrée de la barre latérale, déclarée par `docs/DESIGN_SYSTEM.md` §4 depuis `CRM-000`. Elle reste dans `ROUTES` : son titre est une clé de traduction et son contenu ne dépend d'aucun paramètre de chemin — seule la **portée** vit dans la chaîne de requête (`?qui=tous`). *Elle rendait un état vide inconditionnel depuis `CRM-007` ; sans session, c'est toujours l'état vide qu'elle rend, la RLS ne consentant aucune ligne* |
...
`public.tracks` pour remplir la barre latérale (`docs/SPEC-tracks.md` §7). **Mesuré à l'ouverture
de cette spécification, hors interface :**
...
Ce tableau est la **mesure historique antérieure à `CRM-022`** : les deux appelants obtenaient le
même vide alors que la ligne existait. `CRM-022` a depuis ajouté `workspaces_lecture_membre` :
l'anonyme garde `200` / `[]`, tandis que le vrai jeton lit uniquement ses workspaces.
...
anonyme. Depuis `CRM-022`, un membre lit aussi son workspace, ses memberships et les profils de son
équipe ; INC-014 est close.
...
- **L'administration des identités reste sans écran.** `CRM-022` ferme INC-014 et rend le contexte
  et les personnes lisibles ; `CRM-070` reste propriétaire de l'invitation et de la gestion
  complète des memberships.
