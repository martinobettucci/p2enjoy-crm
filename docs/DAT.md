...
| `scripts/spark/amorcer-espace.sh` | Dans la cellule : premier espace de travail et son administrateur, par la clé de service — compte invité par `generate_link`, sans mot de passe ni courriel ; idempotent | Profil `prod` exigé ; opération d'exploitation, sur instruction explicite ; le lien d'action rendu par GoTrue n'est jamais affiché |
