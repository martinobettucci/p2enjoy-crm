...
- Abonnements Realtime pour les commentaires, les déplacements de cards et les notifications.
  **LIVRÉ POUR LES SEULS COMMENTAIRES depuis `CRM-043`**, et par un mécanisme qu'aucun document ne
  ...
  `public.card_comments` y est ajoutée par la migration 15, et reste **la seule** : `public.notifications`,
  livrée par `CRM-064` tranche 2, n'y est PAS ajoutée tant qu'aucun écran ne l'écoute — sa
  tranche 3 l'y ajoutera dans le même changement que l'abonnement. Les cards restent dues par leur
  unité. `realtime.apply_rls` évalue la politique `SELECT` de la table
...
| Workflows | `workflow_nodes_catalog` (livrée, `CRM-030`), `workflows`, `workflow_steps`, `workflow_transitions` (livrées, `CRM-031`) ; `workflow_transition_required_fields`, liaison normalisée et intègre vers le formulaire (livrée, `CRM-018`) ; vue `workflow_derivations` et fonction `copy_workflow_to_track` (`CRM-032`, redéfinies par `CRM-018` pour copier champs, règles et exigences et comparer une empreinte de composition) ; fonction `move_card`, **garde centrale de transition** (livrée, `CRM-034`, **six vérifications sur six** depuis `CRM-036` qui a refermé INC-047) ; fonction `move_card_to_channel`, **déplacement d'une card d'un graphe à un autre** (livrée, `CRM-045`) ; fonction `change_channel_workflow`, **remappage atomique et exhaustif de toutes les cards d'un channel** (livrée, `CRM-019`) — aucune étape n'est devinée et toute perte de réponse exige `discard_field_values` ; table `workflow_versions`, **photographies immuables** de la composition d'un workflow, et fonctions `app.workflow_composition_document` et `publish_workflow_version` (première tranche de `CRM-078`) — l'immuabilité est opposable jusqu'à la clé de service, et publier ne change rien au comportement du produit ; fonctions `app.composition_collection_diff` et `compare_workflow_versions`, **comparaison de deux versions** (deuxième tranche de `CRM-078`) — `STABLE` et `SECURITY INVOKER`, aucune correspondance n'est devinée, l'identité est faite d'identifiants réels et d'eux seuls ; fonctions `app.workflow_composition_naturel` et `compare_workflow_with_source`, **comparaison d'une copie avec sa source vivante** (dernière tranche de `CRM-032`) — `STABLE` et `SECURITY INVOKER`, elles n'écrivent rien, le §4.1 interdisant toute réapplication automatique ; l'appariement se fait sur les CLÉS NATURELLES du remappage — `node_id`, le couple de nœuds, `key` — et non sur des identifiants, une copie n'en partageant aucun avec sa source ; l'algorithme de différence de `CRM-078` est réutilisé et non réécrit ; fonction `plan_card_remapping`, **plan de remappage des cards avant restauration d'une version** (troisième tranche de `CRM-078`) — `STABLE` et `SECURITY INVOKER`, card par card, aucune destination n'est devinée, les archivées et celles en corbeille comprises, liste bornée dont la troncature est annoncée, et exhaustivité garantie par la réservation aux administrateurs ; fonction `restore_workflow_version`, **application transactionnelle du plan et retour arrière** (quatrième tranche de `CRM-078`) — `VOLATILE` et `SECURITY DEFINER`, seule de l'unité à écrire : elle rejoue le plan dans sa propre transaction plutôt que d'en accepter un pré-calculé, publie la composition vivante comme **point de retour** avant d'écrire — le retour arrière est alors la restauration elle-même appliquée à ce point, donc le même code —, restaure les cinq collections de composition mais **jamais l'identité du workflow**, et **archive** les champs surnuméraires au lieu de les supprimer, `form_fields` ne portant aucune politique `delete` |
...
| Cards | `cards` (livrée, `CRM-040`) ; `card_comments` (livrée, `CRM-043`) ; `card_comment_mentions` (livrée, `CRM-064` tranche 1 — **relation** remplaçant un `uuid[]` sans intégrité ni règle, et le seul endroit du produit où une règle d'accès juge un **tiers** : la chaîne `app.*_pour` généralise les quatre fonctions historiques, qui deviennent des délégations d'une ligne) ; `card_events` (livrée, `CRM-044` — **append-only**, alimentée par cinq triggers, aucune écriture cliente) ; `card_activities`, `card_tags`, `card_watchers`, `card_checklists`, `card_templates` — ces cinq dernières ne sont rattachées à aucune unité |
| Relations | `organizations`, `contacts`, `card_contacts` (livrées, `CRM-060` tranche 1 — modèle et RLS ; cloisonnement `workspace_id` **structurel** par FK composites, aucun trigger de cohérence ; écriture ouverte au `business_developer` **et** à l'`admin` ; `card_contacts` compose `app.can_read_card` / `app.can_write_card`) |
...
| Transverse | `notifications` (**livrée**, `CRM-064` tranche 2 — **produite** par un trigger `AFTER INSERT` sur la mention, `SECURITY DEFINER` par nécessité mesurée ; aucun rôle applicatif n'a de privilège `INSERT` ni `DELETE`, et la mise à jour est bornée à `read_at` par un privilège de **colonne** ; sa lecture **délègue** à `app.can_read_card`, si bien qu'une perte d'accès la masque sans détruire aucune ligne) ; `notification_preferences` (`CRM-064` tranche 4), `audit_log` (`CRM-072`, **unité inexistante**), `api_tokens` (`CRM-073`, **unité inexistante**), `webhook_endpoints`, `webhook_deliveries`, `saved_views` |
...
| Realtime | webapp | Abonnements aux commentaires — **livré `CRM-043`**, `public.card_comments` publiée sur `supabase_realtime`, **seule table publiée à ce jour**. `public.notifications` existe depuis `CRM-064` tranche 2 mais **n'est PAS publiée**, et c'est délibéré : rien ne s'y abonne encore. Elle le sera par la **tranche 3**, dans le même changement que l'écran qui l'écoute — le temps réel évalue la politique `SELECT` de chaque abonné, donc c'est une propriété qui se prouve et non qui s'ajoute par précaution. L'absence est figée par deux assertions. Cards : dues par leur unité |
...
| `public.rendre_modele_email` | rendu d'un modèle sur une affaire, avec l'inventaire des trous nuls (`CRM-063` tranche 2a, migration 56) | `mail_templates`, `cards`, `channels`, `workflow_steps`, `workflow_nodes_catalog`, `contacts`, `organizations`, `mail_outbound_identities` — toutes lues **sous la RLS de l'appelant**, la fonction étant `security invoker` ; **aucune dépendance de service**, et aucun prédicat d'autorisation recopié |
...
ni les fichiers que l'image d'origine attribue à `nobody` (65534), ni le `sudo -u nobody` de ses
migrations n'y fonctionnent. L'image dérivée renumérote `nobody` en 64000, en une seule couche ;
elle est construite sur le poste et chargée dans la cellule (décisions 571 et 575).
...
| `scripts/spark/amorcer-espace.sh` | Dans la cellule : premier espace de travail et l'**attente** de son administrateur, par la clé de service ; aucun compte n'est créé — la personne devient administratrice à sa première connexion LeLabs (`CRM-092` T4, `docs/SPEC-session-sso.md` §12) ; un espace qui a déjà un administrateur n'est pas touché ; idempotent | Profil `prod` exigé ; opération d'exploitation, sur instruction explicite |
